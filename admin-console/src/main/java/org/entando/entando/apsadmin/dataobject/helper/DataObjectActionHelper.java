...
public class DataObjectActionHelper extends EntityActionHelper implements IDataObjectActionHelper {
		...
		try {
			if (null != dataObject) {
				...
				if (null == dataObject.getId()) {
					String mainGroup = request.getParameter("mainGroup");
					if (mainGroup != null) {
						request.getSession().setAttribute(DataObjectActionConstants.SESSION_PARAM_NAME_CURRENT_DATA_OBJECT_GROUP, mainGroup);
					}
				}
	...
	@Override
	public Map getReferencingObjects(DataObject content, HttpServletRequest request) throws ApsSystemException {
		return this.getDataObjectHelper().getReferencingObjects(content);
	}

	@Override
	public ActivityStreamInfo createActivityStreamInfo(DataObject content, int strutsAction, boolean addLink) {
