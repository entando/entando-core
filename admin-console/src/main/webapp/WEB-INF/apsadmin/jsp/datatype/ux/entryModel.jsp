...
<div id="main" role="main">
    <s:form action="save" namespace="/do/dataobject/model"
    ...
    <div class="form-group<s:if test="strutsAction == 1 && null == dataType"> has-warning</s:if>">
        ...
        <div class="col-sm-10">
            <div class="input-group">
                <wpsf:select id="dataType" list="smallDataTypes" name="dataType" listKey="code" listValue="description" cssClass="form-control" headerKey="" headerValue="%{getText('note.choose')}" />
                <span class="input-group-btn">
                    <s:if test="strutsAction == 1 && null == dataType">
                        <wpsf:submit type="button" action="lockDataType" cssClass="btn btn-primary" value="%{getText('label.set')}" />
                    </s:if>
                    <s:else>
                        <wpsf:submit type="button" action="lockDataType" cssClass="btn btn-primary" value="%{getText('label.change')}" />
                    </s:else>
                </span>
            </div>
            <s:if test="strutsAction == 1 && null == dataType">
                <span class="help-block">
                    ...
                    <s:text name="note.dataObjectModel.assist.intro" />
                </span>
            </s:if>
        </div>
    </div>
    ...
    <div class="form-group<s:property value="controlGroupErrorClassVar" />">
        ...
        <label class="col-sm-2 control-label" for="description">
            ...
            <a role="button" tabindex="0" data-toggle="popover" data-trigger="focus" data-html="true" title=""
               data-placement="top" data-content="<s:text name="title.dataObjectModel.name.help" />" data-original-title="">
                <span class="fa fa-info-circle"></span>
            </a>
        </label>
        <div class="col-sm-10">
    ...
    <div class="form-group<s:property value="controlGroupErrorClassVar" />">
        <s:set var="dataObjectShapeFieldErrorsVar"
               value="%{fieldErrors['dataObjectShape']}" />
        ...
        <div class="col-sm-10">
            ...
            <div class="display-block">
                <s:textarea name="dataObjectShape" id="dataObjectShape"  cssClass="form-control" />

                <s:if test="#dataObjectShapeHasFieldErrorVar">
                    <p class="text-danger padding-small-vertical">
                        <s:iterator value="#dataObjectShapeFieldErrorsVar">
                            <s:property />
                        </s:iterator>
                    </p>
                </s:if>
