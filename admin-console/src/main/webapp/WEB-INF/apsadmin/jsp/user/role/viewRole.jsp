...
<ol class="breadcrumb page-tabs-header breadcrumb-position">
    <li><a href="<s:url namespace="/do/BaseAdmin" action="settings" />"><s:text name="menu.configure" /></a>
    <li><a href="<s:url action="list" namespace="/do/Role"></s:url>"
           title="<s:text name="note.goToSomewhere" />: <s:text name="title.roleManagement" />">
            <s:text name="title.roleManagement" /></a></li> 

    <li class="page-title-container"><s:text name="title.roleDetail" /></li> 
</ol>
...
<div class="form-horizontal">
    <div class="form-group">
        <label class="control-label col-sm-2"><s:text name="label.group" /></label>
        <div class="col-sm-10">
            <s:property value="name" />
        </div>
    </div>
    <div class="form-group">
        <label class="control-label col-sm-2"><s:text name="label.description" /></label>
        <div class="col-sm-10">
    ...
    <div class="form-group">
        <label class="control-label col-sm-2"><s:text name="name.permissions" /></label>
        <div class="col-sm-10">
            <s:if test="null != rolePermissions && rolePermissions.size() > 0">
                <ul style="margin:5px 0 0 -40px">
                    <s:iterator value="rolePermissions">
                        <li>
                            <s:property value="description" /> (<s:property value="name" />)
                        </li>
                    </s:iterator>
                </ul>
            </s:if>
            <s:else>
                <p><s:text name="note.role.rolePermissions.empty" /></p>
            </s:else>
        </div>
    </div>
</div>
