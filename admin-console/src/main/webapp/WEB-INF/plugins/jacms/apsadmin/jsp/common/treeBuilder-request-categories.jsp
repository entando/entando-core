...
<tr id="${currentRoot.code}" data-parent="#${currentRoot.parent.code}" 
    class="treeRow <s:if test="%{#currentRoot.code != 'home' && #isHidden}">collapsed childrenNodes</s:if> tree_node_flag ${liClassName}" >
        ...
        <input type="radio" name="${inputFieldName}" 
            ...
            class="subTreeToggler <s:if test="#isSelected">active </s:if> hidden<s:if test="%{#currentRoot.getChildren().length > 0}">  tree_<s:property value="#currentRoot.code" /> </s:if>"
	...
	<td class="text-center">
        <wpsa:actionParam action="joinCategory" var="joinCategoryActionNameVar" >
			<wpsa:actionSubParam name="categoryCode" value="%{#currentRoot.code}" />
		</wpsa:actionParam>
		<wpsf:submit action="%{#joinCategoryActionNameVar}" type="button" 
