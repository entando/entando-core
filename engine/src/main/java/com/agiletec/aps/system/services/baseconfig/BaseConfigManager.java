...
public class BaseConfigManager extends AbstractService implements ConfigInterface {
    ...
    public void init() throws Exception {
        ...
        this.parseParams();
        if (StringUtils.isBlank(this.getKeyString())) {
            this.keyGenerate();
        } else if (this.getKeyString().getBytes().length < 4) {
            throw new RuntimeException("Key String for encryption not properly set: < 4 bytes (it must be between 4 and 56 bytes)");
        } else if (this.getKeyString().getBytes().length > 56) {
...
//        if (props != null & (_oldKeyString = props.getProperty("key.string.encryption")) != null) {
...
//        this.saveParamChanges();
    ...
    public void keyGenerate() throws Exception {
        ...
        try {
            ...
            props.setProperty("admin.user", this.getAdminUser());
            props.setProperty("admin.password", this.getAdminPwd());
    ...
    private void encryptPwd() throws ApsSystemException {
        _logger.info("Password in chiaro: {}", this.getAdminPwd());
        String encryptedPwd = BlowfishApsEncrypter.encryptString(this.getKeyString(), this.getAdminPwd());
        _logger.info("Password criptata: {}", this.getAdminPwd());
    ...
    public void setAdminPwd(String _adminPwd) {
        this._adminPwd = _adminPwd;
    }
