...
public class TableFactory {
    ...
    public <T, ID> TableInfo getTableInfo(Class dataClass) throws ApsSystemException {
        try {
            ...
            if (dao instanceof BaseDaoImpl<?, ?>) {
                return ((BaseDaoImpl<?, ?>) dao).getTableInfo();
            } else {
                return new TableInfo<T, ID>(connectionSource, null, dataClass);
            }
        } catch (Exception e) {
            _logger.error("Error extracting table info", e);
            throw new ApsSystemException("Error extracting table info", e);
        }
    }
