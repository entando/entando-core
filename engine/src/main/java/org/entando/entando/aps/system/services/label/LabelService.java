...
public class LabelService implements ILabelService {
    ...
    private static final String ERRCODE_LABELGROUP_EXISTS = "1";

    private static final String ERRCODE_LABELGROUP_LANGS_DEFAULT_LANG_REQUIRED = "2";

    private static final String ERRCODE_LABELGROUP_LANGS_INVALID_LANG = "3";

    private static final String ERRCODE_LABELGROUP_LANGS_TEXT_REQURED = "4";
    ...
    @Override
    public PagedMetadata<LabelDto> getLabels(RestListRequest restRequest) {
    ...
    protected BeanPropertyBindingResult validateAddLabelGroup(LabelDto labelDto) {
        try {
            ...
            // 1) NON deve esistere
            if (null != labelGroup) {
                bindingResult.reject(ERRCODE_LABELGROUP_EXISTS, new String[]{code}, "labelGroup.code.already.present");
            ...
            if (!labelDto.getLanguages().keySet().contains(defaultLang)) {
                bindingResult.reject(ERRCODE_LABELGROUP_LANGS_DEFAULT_LANG_REQUIRED, new String[]{defaultLang}, "labelGroup.langs.defaultLang.required");
                return bindingResult;
            }
    ...
    private void validateLangEntry(Entry<String, String> entry, List<String> configuredLangs, String defaultLangCode, BeanPropertyBindingResult bindingResult) {
        ...
        if (!configuredLangs.contains(currentLangCode)) {
            bindingResult.reject(ERRCODE_LABELGROUP_LANGS_INVALID_LANG, new String[]{currentLangCode}, "labelGroup.langs.lang.invalid");
        }
        ...
        if (currentLangCode.equals(defaultLangCode) && StringUtils.isBlank(entry.getValue())) {
            bindingResult.reject(ERRCODE_LABELGROUP_LANGS_TEXT_REQURED, new String[]{currentLangCode}, "labelGroup.langs.defaultLang.textRequired");
        }
