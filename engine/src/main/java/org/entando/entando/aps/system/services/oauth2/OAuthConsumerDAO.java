...
public class OAuthConsumerDAO extends AbstractSearcherDAO implements IOAuthConsumerDAO {
    ...
    private int fillStatement(ConsumerRecordVO consumer, int index, PreparedStatement stat) throws SQLException {
        // consumersecret, name, description, callbackurl,scope, authorizedgranttypes, expirationdate, issueddate
        stat.setString(index++, consumer.getSecret());
        stat.setString(index++, consumer.getName());
        stat.setString(index++, consumer.getDescription());
        stat.setString(index++, consumer.getCallbackUrl());
        stat.setString(index++, consumer.getScope());
        stat.setString(index++, consumer.getAuthorizedGrantTypes());
        if (null != consumer.getExpirationDate()) {
            stat.setDate(index++, new Date(consumer.getExpirationDate().getTime()));
        } else {
            stat.setNull(index++, Types.DATE);
        }
        if (null != consumer.getIssuedDate()) {
            stat.setDate(index++, new Date(consumer.getIssuedDate().getTime()));
        } else {
            stat.setNull(index++, Types.DATE);
        }
        return index;
    ...
    private String UPDATE_CONSUMER
            = "UPDATE api_oauth_consumers SET consumerkey = ?, consumersecret = ? , name=?, description = ? , callbackurl = ?, scope=?, authorizedgranttypes = ?, expirationdate = ?, issueddate = ? WHERE consumerkey = ? ";
